...
def addWorkitem():
    ...
    cursor.execute('insert into record_work_item(customer_id, part_no,lot_no,creator,create_dt,test_style) values (?, ?,?,?,?,?)', [
                   customer_id, part_no, lot_no, creator, create_dt, test_style])
    conn.commit()

    cursor.execute(
        "select * from record_work_item order by create_dt desc limit 1")
    tables = cursor.fetchall()
    ...
    cursor.execute('insert into record_prepare(type, tool,work_item_id,start_dt) values (?, ?,?,?)', [
                   prepare_type, prepare_tool, work_item_id, create_dt])
    conn.commit()

    cursor.execute('insert into record_test(work_item_id,create_dt) values (?,?)', [
                   work_item_id, create_dt])
    conn.commit()
...
def changePrepareRecordPhase():
    ...
    cursor.execute('update record_prepare set status = "closed" , end_dt = "' +
                   end_dt+'"  where record_id = '+record_id)
    conn.commit()

    cursor.execute('insert into record_prepare(type, tool,work_item_id,start_dt) values (?, ?,?,?)', [
                   next_type, next_tool, work_item_id, create_dt])
    conn.commit()

    if next_type == "完成":
        cursor.execute('update record_work_item set prepare_end_dt = "' +
                       end_dt+'" where work_item_id = '+work_item_id)
        conn.commit()
...
def finishPrepareRecordPhase():
    ...
    if next_type == "完成":
        cursor.execute('update record_work_item set prepare_end_dt = "' +
                       end_dt+'", status="待處理" where work_item_id = '+work_item_id)
        conn.commit()
...
def checkInPrepareRecord():
    ...
    cursor.execute('insert into record_prepare(type, tool,work_item_id,start_dt) values (?, ?,?,?)', [
                   current_type, select_tool_id, work_item_id, create_dt])
    conn.commit()

    if current_type == "工程":
        cursor.execute('update record_work_item set prepare_start_dt = "' +
                       create_dt+'"  where work_item_id = '+work_item_id)
        conn.commit()
...
def getTools(prepare_or_test):
    ...
    search_sql = "select * from tool_info where prepare_or_test = '" + \
        prepare_or_test+"' and tool_id <> '待處理'"
...
def checkInTestRecord():
    ...
    cursor.execute(
        "select * from record_test where test_record_id = "+record_id)
    tables = cursor.fetchall()

    work_item_id = tables[0][1]

    cursor.execute('update record_test set test_tool_id = "'+select_tool_id +
                   '" , test_start_dt = "'+start_dt+'"  where test_record_id = '+record_id)
    conn.commit()

    cursor.execute('update record_work_item set  test_start_dt = "' +
                   start_dt+'", status = "測試中" where work_item_id = '+work_item_id)
    conn.commit()
...
def checkInFlyTestRecord():
    ...
    for tool_id in select_tools:
        ...
        if tool_count > 1:
            ...
            cursor.execute('insert into record_test(work_item_id,test_tool_type,test_tool_id,create_dt) values (?,?,?,?)', [
                           work_item_id, test_tool_type, tool_id, create_dt])
            conn.commit()
        else:
            cursor.execute('update record_test set test_tool_id = "'+tool_id +
                           '" , test_start_dt = "'+start_dt+'"  where test_record_id = '+record_id)
            conn.commit()
...
def finishTestRecord():
    ...
    cursor.execute('update record_test set test_end_dt = "' +
                   end_dt+'"  where test_record_id = '+record_id)
    conn.commit()

    cursor.execute('update record_work_item set  test_end_dt = "' +
                   end_dt+'", status = "測試完成待包裝ㄔ" where work_item_id = '+work_item_id)
    conn.commit()
...
def getCustomerList(tam_alias):
    ...
    if tam_alias == "null":
        ...
        if permission == "guest":
            tam_alias = "Demo"
            search_sql = "select a.*, w.cnt, e.sentiment from (select c.*, s.* from tam_info t, customer_survey_overall s, customer_info c  where ((t.esm_top like '"+tam_alias+"' or t.esm like '"+tam_alias+"' )  or c.tam_1 like '"+tam_alias + \
                "') and t.tam_alias = c.tam_1 and c.customer_name = s.customer_name) a LEFT JOIN (select customer_name, count(workload_id) as cnt from customer_workload group by customer_name ) w  on  a.customer_name = w.customer_name LEFT JOIN customer_experience e  on  w.customer_name = e.customer_name and e.latest like 'Y'"
...
def getCommunicationLog(customer_name):
    ...
    search_sql = "select * from communication_log where customer_name like '" + \
        customer_name+"' order by seq desc"
...
def checkUserPermission(user_alias):
    ...
    cursor.execute(
        "select permission from acct_permission where acct_alias like '"+user_alias+"'")
    tables = cursor.fetchall()
...
def getMemberList(tam_alias):
    ...
    if tam_alias == "null":
        ...
        if permission == "guest":
            ...
            print("default + pilot")
            query_sql = "select distinct tam_alias from tam_info where esm like '" + \
                request.remote_user+"' order by tam_alias"
        ...
        #   print(tam_alias + " + " + request.remote_user)
        cursor.execute(
            "select distinct  esm from tam_info where esm_top like '"+tam_alias+"' order by esm")
        tables = cursor.fetchall()
        ...
        if len(tables) > 0:
            ...
            cursor.execute(
                "select distinct tam_alias from tam_info where esm like '"+tam_alias+"' order by tam_alias")
            tables = cursor.fetchall()
            result = {}
            result["results"] = []
...
def getTAMInfo(tam_alias):
    ...
    if tam_alias == "null":
        ...
        if permission == "guest":
            ...
            cursor.execute(
                "select * from tam_info where tam_alias like '"+request.remote_user+"'")
            tables = cursor.fetchall()
        ...
        if permission == "admin":
            ...
            cursor.execute(
                "select * from tam_info where tam_alias like '"+tam_alias+"'")
...
def getCustomerInfo(customer_name):
    ...
    cursor.execute(
        "select * from customer_info where customer_name like '"+customer_name+"'")
    tables = cursor.fetchall()
...
def getCustomerSurveySummary(customer_name):
    ...
    cursor.execute(
        "select * from customer_survey_overall where customer_name like '"+customer_name+"'")
    tables = cursor.fetchall()
    if len(tables) == 0:
        cursor.execute('insert into customer_survey_overall(customer_name, summary) values (?, ?)', [
                       customer_name, "please input TAM's summary for the customer"])
        conn.commit()
        cursor.execute(
            "select * from customer_survey_overall where customer_name like '"+customer_name+"'")
        tables = cursor.fetchall()
    ...
    result["scores"].append(
        {"catg": "Business Continuity by Workload", "type": "5star", "score": q1_score})
    result["scores"].append(
        {"catg": "People Readiness", "type": "5star", "score": q2_score})
    result["scores"].append(
        {"catg": "Cost Optimization", "type": "5star", "score": q3_score})
    result["scores"].append(
        {"catg": "Technology Innovation", "type": "5star", "score": q4_score})
    result["scores"].append(
        {"catg": "Security", "type": "5star", "score": q5_score})
...
def getCustomerSurveyAnswer(customer_name):
    ...
    cursor.execute("select * from (select * from customer_info where customer_name like '"+customer_name +
                   "') i left join (select * from customer_survey_answer where latest='Y') a on i.customer_name=a.customer_name ")
    tables = cursor.fetchall()
...
def get_CustomerSurveyResult(customer_name):
    ...
    cursor.execute(
        "select * from customer_workload where customer_name like '"+customer_name+"'")
    ...
    cursor.execute(
        "select * from workload_survey where customer_name like '"+customer_name+"'")
    workload_survey_results = cursor.fetchall()
    if len(workload_survey_results) == 0:
        for workload in workloads:
            cursor.execute('insert into workload_survey(customer_name, workload_id) values (?, ?)', [
                           customer_name, workload[2]])
            conn.commit()
            cursor.execute(
                "select * from workload_survey where customer_name like '"+customer_name+"'")
            workload_survey_results = cursor.fetchall()
    ...
    cursor.execute(
        "select * from workload_survey_comment  where customer_name like '"+customer_name+"'")
    workload_survey_comments = cursor.fetchall()
    if len(workload_survey_comments) == 0:
        for workload in workloads:
            cursor.execute('insert into workload_survey_comment(customer_name, workload_id) values (?, ?)', [
                           customer_name, workload[2]])
            conn.commit()
            cursor.execute(
                "select * from workload_survey_comment  where customer_name like '"+customer_name+"'")
    ...
    for workload in workloads:
        ...
        for question in questions:
            ...
            q["Rating_Definition"] = []
            q["Rating_Definition"].append(
                {"rate": "5 stars", "value": 5, "checked": False, "def": question[6]})
            q["Rating_Definition"].append(
                {"rate": "4 stars", "value": 4, "checked": False, "def": question[7]})
            q["Rating_Definition"].append(
                {"rate": "3 stars", "value": 3, "checked": False, "def": question[8]})
            q["Rating_Definition"].append(
                {"rate": "2 stars", "value": 2, "checked": False, "def": question[9]})
            q["Rating_Definition"].append(
                {"rate": "1 stars", "value": 1, "checked": False, "def": question[10]})
    ...
    for question in questions:
        ...
        q["Rating_Definition"] = []
        q["Rating_Definition"].append(
            {"rate": "5 stars", "value": 5, "checked": False, "def": question[7]})
        q["Rating_Definition"].append(
            {"rate": "4 stars", "value": 4, "checked": False, "def": question[8]})
        q["Rating_Definition"].append(
            {"rate": "3 stars", "value": 3, "checked": False, "def": question[9]})
        q["Rating_Definition"].append(
            {"rate": "2 stars", "value": 2, "checked": False, "def": question[10]})
        q["Rating_Definition"].append(
            {"rate": "1 stars", "value": 1, "checked": False, "def": question[11]})
    ...
    cursor.execute("select * from customer_survey_result where customer_name = '" +
                   customer_name+"'  and latest like 'Y'")
    tables = cursor.fetchall()
    print(tables)
    if len(tables) == 0:
        cursor.execute(
            'insert into customer_survey_result(customer_name) values (?)', [customer_name])
        conn.commit()
        cursor.execute("select * from customer_survey_result where customer_name = '" +
                       customer_name+"'  and latest like 'Y'")
        tables = cursor.fetchall()
    ...
    cursor.execute(
        "select * from customer_survey_comment where customer_name = '"+customer_name+"'")
    tables = cursor.fetchall()
    if len(tables) == 0:
        cursor.execute(
            'insert into customer_survey_comment(customer_name) values (?)', [customer_name])
        conn.commit()
        cursor.execute(
            "select * from customer_survey_comment where customer_name = '"+customer_name+"'")
        tables = cursor.fetchall()
...
def getCustomerESStatus(customer_name):
    ...
    # if len(tables) == 0 :
    cursor.execute("select * from customer_es_status_result where customer_name like '" +
                   customer_name+"' and latest = 'Y' ")
    tables = cursor.fetchall()
...
def updateCustomerSummary():
    ...
    conn = sqlite3.connect(DATABASE)
    cursor = conn.cursor()
    cursor.execute('update customer_survey_overall set summary = "'+summary +
                   '" , update_dt = "'+today+'"  where customer_name like "'+customer_name+'"')
    conn.commit()
    result = {}
    result["result"] = "success"
    return jsonify(result)
...
def updateCatgSummary():
    ...
    if "_catg_" in catg:
        ...
        conn = sqlite3.connect(DATABASE)
        cursor = conn.cursor()
        cursor.execute('update workload_survey set summary = "'+summary+'" , update_dt = "' +
                       today+'"  where customer_name like "'+customer_name+'" and workload_id = ' + workload_id)
        conn.commit()

    else:
        conn = sqlite3.connect(DATABASE)
        cursor = conn.cursor()
        cursor.execute('update customer_survey_result set '+catg_id+'_summary = "' +
                       summary+'" where customer_name like "'+customer_name+'"  and latest like "Y"')
        conn.commit()
...
def updateQuestionDesc():
    ...
    if "_q_" in question:
        ...
        conn = sqlite3.connect(DATABASE)
        cursor = conn.cursor()
        cursor.execute('update workload_survey_comment set '+question_id+'_desc = "'+desc+'" , update_dt = "' +
                       today+'"  where customer_name like "'+customer_name+'" and workload_id='+workload_id+'')
        conn.commit()
        ...
        conn = sqlite3.connect(DATABASE)
        cursor = conn.cursor()
        cursor.execute('update customer_survey_comment set '+question+'_desc = "'+desc +
                       '"  , update_dt = "'+today+'"  where customer_name like "'+customer_name+'"')
        conn.commit()
    ...
    if checkCompleteComment(customer_name):
        cursor.execute(
            'update customer_survey_overall set complete_comment = "Yes"  where customer_name like "'+customer_name+'"')
        conn.commit()
...
def updateQuestionAction():
    ...
    if "_q_" in question:
        ...
        conn = sqlite3.connect(DATABASE)
        cursor = conn.cursor()
        cursor.execute('update workload_survey_comment set '+question_id+'_action = "'+action+'" , update_dt = "' +
                       today+'"  where customer_name like "'+customer_name+'" and workload_id='+workload_id+'')
        conn.commit()
        ...
        conn = sqlite3.connect(DATABASE)
        cursor = conn.cursor()
        cursor.execute('update customer_survey_comment set '+question+'_action = "'+action +
                       '"  , update_dt = "'+today+'"  where customer_name like "'+customer_name+'"')
        conn.commit()
    # print(customer_name  + ", "+ summdescary)
...
def updateQuestionScore():
    ...
    if "_q_" in question:
        ...
        conn = sqlite3.connect(DATABASE)
        cursor = conn.cursor()
        cursor.execute('update workload_survey set '+question_id+' = '+score+' , update_dt = "' +
                       today+'" where customer_name like "'+customer_name+'" and workload_id='+workload_id+'')
        conn.commit()
        print('update workload_survey set '+question_id+' = '+score+' , update_dt = "'+today +
              '" where customer_name like "'+customer_name+'" and workload_id='+workload_id+'')
        cursor.execute(
            'select * from workload_survey where customer_name like "'+customer_name+'"')
        workload_survey_results = cursor.fetchall()
        ...
        cursor.execute('update customer_survey_overall set q1 = '+str(workload_avg) +
                       ' , update_dt = "'+today+'" where customer_name like "'+customer_name+'"')
        conn.commit()
        ...
        conn = sqlite3.connect(DATABASE)
        cursor = conn.cursor()
        cursor.execute('update customer_survey_result set '+question+' = '+str(score)+', update_dt = "' +
                       today+'"  where customer_name like "'+customer_name+'"  and latest like "Y"')
        conn.commit()
    ...
    cursor.execute('select * from customer_survey_result where customer_name like "' +
                   customer_name+'"  and latest like "Y"')
    ...
    cursor.execute('update customer_survey_overall set '+question.split("_")[0]+' = '+str(
        workload_avg)+' , update_dt = "'+today+'" where customer_name like "'+customer_name+'"')
    print('update customer_survey_overall set '+question.split("_")[0]+' = '+str(
        workload_avg)+' , update_dt = "'+today+'" where customer_name like "'+customer_name+'"')
    conn.commit()
    ...
    if checkCompleteSurvey(customer_name):
        cursor.execute(
            'update customer_survey_overall set complete_survey = "Yes"  where customer_name like "'+customer_name+'"')
        conn.commit()
...
def refreshOverallSummary(customer_name):
    ...
    for question in questions:
        ...
        cursor.execute('update customer_survey_overall set '+question+' = '+str(workload_avg) +
                       ' , update_dt = "'+today+'" where customer_name like "'+customer_name+'"')
        print('update customer_survey_overall set '+question+' = '+str(workload_avg) +
              ' , update_dt = "'+today+'" where customer_name like "'+customer_name+'"')
        conn.commit()
...
def checkCompleteSurvey(customer_name):
    ...
    # workload survey
    cursor.execute('select * from workload_survey where customer_name like "' +
                   customer_name+'"  and latest like "Y"')
    survey_result = cursor.fetchall()
...
def checkCompleteComment(customer_name):
    ...
    cursor.execute('select * from customer_survey_comment where customer_name like "' +
                   customer_name+'"  and latest like "Y"')
    survey_result = cursor.fetchall()
...
def addWorkload():
    ...
    cursor.execute("select * from customer_workload where customer_name like '" +
                   customer_name+"' and workload_name like '"+workload_name+"'")
    tables = cursor.fetchall()
    ...
    if len(tables) > 0:
        ...
        cursor.execute('insert into customer_workload(customer_name, workload_name) values (?, ?)', [
                       customer_name, workload_name])
        conn.commit()
...
def changeWorkloadName():
    ...
    cursor.execute('update customer_workload set workload_name = "'+workload_name +
                   '" where customer_name like "'+customer_name+'" and workload_id = '+workload_id)
    conn.commit()
...
def updateWorkload():
    ...
    cursor.execute(cursor.execute('update customer_workload set workload_name = "'+new_workload_name +
                                  '"   where customer_name like "'+customer_name+'" and workload_id = '+workload_id))
    conn.commit()
...
def submitSurvey():
    ...
    cursor.execute("select * from customer_survey_answer where customer_name like '" +
                   customer_name+"' and latest like 'Y'")
    tables = cursor.fetchall()

    if len(tables) > 0:

        cursor.execute('update customer_survey_answer set latest="N" where customer_name like "' +
                       customer_name+'" and latest like "Y"')
        conn.commit()
    ...
    if len(tables) > 0:
        ...
        cursor.execute('update customer_survey_result set latest="N" where customer_name like "' +
                       customer_name+'" and latest like "Y"')
        conn.commit()
    ...
    if len(tables) == 0:
        cursor.execute('insert into customer_survey_comment(customer_name,update_dt) values (?,?)', [
                       customer_name, today])
        conn.commit()
    ...
    for workload in q1["list"]:
        ...
        print("select * from workload_survey_answer where customer_name like '" +
              customer_name+"' and workload_id = "+workload_id+" and latest like 'Y'")
        cursor.execute("select * from workload_survey_answer where customer_name like '" +
                       customer_name+"' and workload_id = "+workload_id+" and latest like 'Y'")
        ...
        if len(tables) > 0:
            ...
            cursor.execute('update workload_survey_answer set latest="N" where customer_name like "' +
                           customer_name+'" and workload_id = '+workload_id+'  and latest like "Y"')
            conn.commit()

        cursor.execute('insert into workload_survey_answer(customer_name, workload_id,a1_1,a1_2,a1_3,a1_4,update_dt) values (?,?,?,?,?,?,?)', [
                       customer_name, workload_id, a1_1, a1_2, a1_3, a1_4, today])
        conn.commit()
        ...
        if len(tables) > 0:
            ...
            cursor.execute('update workload_survey set latest="N" where customer_name like "' +
                           customer_name+'" and workload_id = '+workload_id+'  and latest like "Y"')
            conn.commit()
            print('update workload_survey set latest="N" where customer_name like "' +
                  customer_name+'" and workload_id = '+workload_id+'  and latest like "Y"')

        cursor.execute('insert into workload_survey(customer_name, workload_id,summary,q1_1,q1_2,q1_3,q1_4,update_dt) values (?,?,?,?,?,?,?,?)', [
                       customer_name, workload_id, q1_summary, q1_1, q1_2, q1_3, q1_4, today])
        conn.commit()
        ...
        cursor.execute("select * from workload_survey_comment where customer_name like '" +
                       customer_name+"' and workload_id = "+workload_id+" and latest like 'Y'")
        tables = cursor.fetchall()

        if len(tables) == 0:
            cursor.execute('insert into workload_survey_comment(customer_name, workload_id,update_dt) values (?,?,?)', [
                           customer_name, workload_id, today])
            conn.commit()

    if workload_count > 0:
        workload_avg = workload_sum // workload_count
        cursor.execute('update customer_survey_overall set q1 = "' +
                       str(workload_avg)+'"  where customer_name like "'+customer_name+'"')
        conn.commit()
    print("workload_avg = " + str(workload_avg))

    cursor.execute('update customer_info set es_type = "' +
                   es_type+'"  where customer_name like "'+customer_name+'" ')
    conn.commit()
...
def updateCustomerESStatus():
    ...
    tables = cursor.fetchall()
    if len(tables) == 0:
        cursor.execute('insert into customer_es_status_result(customer_name, '+question_id+', '+question_id +
                       '_comment, '+question_id+'_update_dt) values (?, ?, ?, ?)', [customer_name, answer, comment, update_dt])
        conn.commit()
    else:
        cursor.execute('update customer_es_status_result set '+question_id+' = "'+answer+'" , '+question_id+'_comment = "' +
                       comment+'", '+question_id+'_update_dt = "'+update_dt+'"   where customer_name like "'+customer_name+'"')
        conn.commit()
    result = {}
    result["result"] = "success"
    return jsonify(result)
...
def getCustomerExperience(customer_name):
    ...
    cursor.execute("select * from customer_experience where customer_name like '" +
                   customer_name+"' and latest = 'Y'")
    tables = cursor.fetchall()
    if len(tables) == 0:
        cursor.execute('insert into customer_experience(customer_name, update_dt ) values (?, ?)', [
                       customer_name, update_dt])
        conn.commit()
...
def updateSentiment():
    ...
    tables = cursor.fetchall()
    if len(tables) == 0:
        cursor.execute('insert into customer_experience(customer_name, sentiment, update_dt ) values (?, ?,?)', [
                       customer_name, score, update_dt])
        conn.commit()
    else:
        summary = tables[0][3]
        cursor.execute('update customer_experience set latest = "N"   where customer_name like "' +
                       customer_name+'" and latest = "Y"')
        conn.commit()

        cursor.execute('insert into customer_experience(customer_name, sentiment, update_dt, sentiment_summary ) values (?, ?,?,?)', [
                       customer_name, score, update_dt, summary])
        conn.commit()
    result = {}
    result["result"] = "success"
    return jsonify(result)
