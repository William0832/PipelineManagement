    ...
    $.ajax({
        ...
        success: function (json) {
            ...
            $.each(json.results, function (i, result) {
                ...
                var panel = $('<panel>')
                var table = $('<table>')
                var button = $('<button onclick="location.href=\'/pages/Record_Test.html\'">紀錄</button>')
                table.append($('<tr>').append($('<td>').append($('<label>').text(tool_id))).append($('<td>').css("padding-left","15px").append($('<label>').text('測試人員'))).append($('<td>').text(owner)));
                
                if(record_id){
                    panel.addClass("panel_working");
                    table.append($('<tr>').append($('<td>').append($('<label>').text('測試中料號: '))).append($('<td>').text(part_no)));
